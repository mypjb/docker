...
COMMON_ARGS=" run -d --restart=always --net=${NETWORK} -v /etc/resolv.conf:/etc/resolv.conf "
#docker network create -d bridge $NETWORK

##mysql

#docker volume create mysql-conf && docker volume create mysql-data
#docker $COMMON_ARGS --name mysql -v mysql-conf:/etc/mysql -v mysql-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${USER} -p 3306:3306 $MYSQL_TAG
...
#docker $COMMON_ARGS --name nginx -v nginx:/etc/nginx $COMMON_ARGS -p 80:80 -p 443:443 $NGINX_TAG
#cp -r -f nginx/* ${DOCKER_VOLUME_PATH}/nginx/_data/
#docker restart nginx

##nuget
#docker volume create baget
#docker $COMMON_ARGS --name nuget-server -p 8082:80 --env-file baget/baget.env -v baget:/var/baget loicsharma/baget:latest

##dnsmasq

#docker volume create dnsmasq
#docker $COMMON_ARGS --name=dns -p 53:53/tcp -p 53:53/udp -v dnsmasq:/etc $DNS_TAG
