...
image_commands[0]="ctr -n ${k8s_np} image pull ${image_url}/sig-storage-csi-snapshotter:v4.2.0 && ctr -n ${k8s_np} image tag  ${image_url}/sig-storage-csi-snapshotter:v4.2.0 ${k8s_np}/sig-storage/csi-snapshotter:v4.2.0"

image_commands[1]="ctr -n ${k8s_np} image pull ${image_url}/sig-storage-csi-node-driver-registrar:v2.3.0 && ctr -n ${k8s_np} image tag  ${image_url}/sig-storage-csi-node-driver-registrar:v2.3.0 ${k8s_np}/sig-storage/csi-node-driver-registrar:v2.3.0"
